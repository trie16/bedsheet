...
if ($_GET["module"]=='i_sprei'){
	...
	$sql="SELECT * FROM `t_sprei` WHERE ID_USER = $j[id];";
	$res=mysql_query($sql,$link);
    $jmldata = mysql_num_rows($res);
    echo "<p align=center>Total available data : $jmldata <br>"; 	  			  		  	  
	$tampil=mysql_query("SELECT * FROM t_sprei WHERE ID_USER = $j[id] ORDER BY ID_SPREI DESC limit $posisi,$batas");
	$n=mysql_num_rows($tampil);		
	
	?>
	<table align=center>
    <tr><th>no</th><th>merk</th><th>ukuran</th><th>keterangan</th><th>action</th></tr>
	<?php	
	$no=$posisi+1;
	...
	?>
		...
		<tr>
		<td><?php echo $no; ?></td>
		<td><a href=?module=detailsprei&id=<?php echo $r['ID_SPREI'] ?>><?php echo $join1['NAMA']; ?></a></td>
		<td><?php echo $join2['UKURAN']; ?></td>
		<td><?php echo $r['DESKRIPSI']; ?></td>
		<td><a href=?module=editsprei&id=<?php echo $r['ID_SPREI'];?>>Edit</a> |
...
<?php
	...
	$sql="select t_merk.ID_MERK, t_sprei.ID_MERK, t_ukuran.ID_UKURAN, t_sprei.ID_UKURAN, t_sprei.ID_SPREI as id_s, 
		  t_merk.NAMA as brand, t_ukuran.UKURAN as ukur, t_sprei.GAMBAR as gbr
...
//	echo $sql;
	...
	if ($jumlah > 0){ 
		...
		?>
		<table align="center">
		<tr><th>no</th><th>merk</th><th>ukuran</th><th>gambar</th><th>action</th></tr>
		<?php
	    while ($r=mysql_fetch_array($res)){ ?>
			<tr>
			<td><?php echo $no; ?></td>
          	<td><a href=?module=detailsprei&id=<?php echo $r['id_s']; ?>><?php echo $r['brand']; ?></a></td>
		  	<td><?php echo $r['ukur']; ?></td>
		  	<td><?php echo $r['gbr']; ?></td>
			<td><a href=?module=editsprei&id=<?php echo $r['id_s'];?>>Edit</a> |
...
elseif ($_GET["module"] == 'add_sprei') {
	$link=koneksi_db();
//	$sql="SELECT * FROM t_sprei WHERE ID_SPREI LIKE '%$_POST[kata]%' OR MERK LIKE '%$_POST[kata]%' OR DESKRIPSI LIKE '%$_POST[kata]%'";
